...
describe("Tile object test cases", () => {
    ...
    it("(Positive) Tile object creation", () => {
        expect(() => new Tile("test_name", {}, [0, 1, 2, 3])).not.toThrow(Error);
        expect(() => new Tile("", {}, [0, 1, 2, 3])).not.toThrow(Error);
    });

    it("(Negative) Tile object creation", () => {
        expect(() => new Tile("test_name", null, [0, 1, 2, 3])).toThrow(Error);
        expect(() => new Tile("test_name", {}, [0, 1, 2])).toThrow(Error);
    });

    it("(Positive) toString function", () => {
        let tile = new Tile("Tile_None", {}, [0, 1, 1, 0]);
    ...
    it("(Positive) isCompatible function", () => {
        let tile = new Tile("Tile_None", {}, [1, 1, 1, 0]);
        let tile2 = new Tile("Tile_", {}, [1, 0, 1, 0]);
        expect(Tile.isCompatible(tile, 2, tile2)).toBe(true);
    });

    it("(Negative) isCompatible function", () => {
        let tile = new Tile("Tile_None", {}, [1, 1, 1, 0]);
        let tile2 = new Tile("Tile_", {}, [0, 0, 1, 0]);
